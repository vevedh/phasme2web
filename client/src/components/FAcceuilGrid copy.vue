...
export default defineComponent({
    ...
    computed: {
        ...
        nbRdc() {
            return this.liveDatas.filter((item) => item.typeacces == 'RDC' && item.etat != 'efface' ).length;
        },
        nbParking() {
            return this.liveDatas.filter((item) => item.typeacces == 'PARKING' && item.etat != 'efface')
                .length; // this.liveDatas.filter(item => item.typeacces == "PARKING" && item.etat == "PRESENT" ).length
        },
        nbResParking() {
    ...
    methods: {
        ...
        async updateDatas() {
            let tmpFormDatas;
            let formFields;
            const tmpdatas = await feathersClient.service('tables').find({
                query: {
                    tableDb: this.dbtype,
                    tableName: this.dbname,
            ...
            this.data = Object.assign([], tmpdatas.filter(
