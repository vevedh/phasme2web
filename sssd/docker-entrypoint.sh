...
# Exemple d'exécution d'une commande avec redirection des erreurs vers le fichier de log
#commande_qui_peut_echouer 2>> "$LOGFILE" || log "Erreur lors de l'exécution de la commande"
...
DC_IPADDRESS=${DC_IPADDRESS:-192.168.136.100}
...
echo --------------------------------------------------
echo "Sauvegarde de la conf smb.conf.original"
echo --------------------------------------------------
...
crudini --set /etc/sssd/sssd.conf "domain/${DOMAIN_NAME^^}" "ad_server" "${DOMAIN_NAME^^}"
crudini --set /etc/sssd/sssd.conf "domain/${DOMAIN_NAME^^}" "krb5_realm" "$(echo ${ADMIN_SERVER,,} | awk '{print $1}')"
#crudini --set /etc/sssd/sssd.conf "domain/${DOMAIN_NAME^^}" "cache_credentials" "False"
...
cat <<EOL >  /etc/security/pam_winbind.conf 
[global]
      krb5_auth = yes
...
crudini --set $SAMBA_CONF global "client ldap sasl wrapping" "sign"
