<template>
  <div class="q-px-sm q-form-checkbox q-form-builder-element" >
    <div class="row">
    ...
    <div class="row">
      <q-list dense>
        ...
        <q-item dense v-if="includeOther">
          <q-item-section avatar><q-checkbox v-model="selectedOther" label="Autre :" keep-color :color="hasError ? 'red' : ''" @input="onUpdate" :ref="`checkbox-other-${id}`" /></q-item-section>
      ...
      if (this.required) {
        if (!this.innerValue.length && this.allOptions.length) {
          ...
          if (this.allOptions.length > 1) {
            this.errors.push('Svp choisissez une option.')
          } else {
            this.errors.push('Svp sélectionnez l\'option requise.')
          }
        }
      }
    }
