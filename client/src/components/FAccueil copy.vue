<template>
  <q-splitter class="compAccueil" v-model="splitterModel" style="height: 100%">
    <template v-slot:before>
      ...
      >
        <q-tab
          content-class="text-primary q-tab__bindicator"
          name="formulaire"
          icon="feed"
          label="Formulaire"
        />
        <q-tab content-class="text-teal q-tab__bindicator" name="v_donnees" icon="list_alt"  label="Le Registre" />
        <!--<q-btn-dropdown
          auto-close
          stretch
          flat
          :class="
            String(tvue).substr(0, 2) == 'v_'
              ? 'mnreg_select mnreg_border q-py-sm full-width'
              : 'q-py-sm full-width'
          "
        >
          <template v-slot:label>
            <div>
              <div>
                ...
                <div class="row items-center no-wrap">Registre...</div>
              </div>
            </div>
          </template>

          <q-list>
            <q-item clickable @click="tvue = 'v_donnees'">
              <q-item-section>Visites</q-item-section>
            </q-item>

            <q-item clickable @click="tvue = 'photos'">
              <q-item-section>Accès</q-item-section>
            </q-item>
            <q-item clickable @click="tvue = 'photos'">
              <q-item-section>Par jour</q-item-section>
            </q-item>
            <q-item clickable @click="tvue = 'photos'">
              <q-item-section>Parking</q-item-section>
            </q-item>
            <q-item clickable @click="tvue = 'photos'">
              <q-item-section>RDC</q-item-section>
            </q-item>
          </q-list>
        </q-btn-dropdown>-->
      </q-tabs>
    ...
    <template v-slot:after>
      ...
      >
        <q-tab-panel name="formulaire" class="q-pt-sm">
        ...
        <q-tab-panel name="v_donnees" class="q-pa-none">
          <f-acceuil-table
            :ref="'myTable'"
