            ...
            <template v-slot:top="props">
                <div class="col-12 col-md full-width ">
                    <div class="row q-gutter-sm text-h6 q-px-sm ">
                        ...
                        <span>&nbsp;</span>-->
                        <span class="row items-center text-h8  q-pa-none">
                            Véhicules au PARKING &nbsp;<q-avatar color="teal" size="35px" font-size="20px"  text-color="white" >{{
                                    nbParking
                            }}</q-avatar>
                        </span>
        ...
        <q-dialog v-model="show_EditDialog">
                            <q-card style="width: 80%; max-width: 100%">
                                ...
                                <q-card-section class="q-pa-md" style="width: 100%; max-width: 100%">
                                    <f-vv-form-by-name ref="editForm" :nameForm="(selectedRow.typeacces=='PARKING')?'Demande d\'accès Parking':'Accueil'"  :fieldsmodel="selectedRow"
...
export default defineComponent({
    ...
    computed: {
        ...
        visiteursDuJour() {
            ...
            return this.$store.state.admin.visiteurs
                .filter(
                    (elt) => (elt.datevisite == this.datedujour) && (elt.etat != 'attente') && (elt.etat != 'efface')
                )
                .sort((a, b) => {
 ...
 console.log('Table result info entre parking :', tmpdatas.filter((o) => o.typeacces=='PARKING'));
    ...
    beforeMount() {
        console.log('Store context', this.$FeathersVuex);
        ...
        if (this.dbname && this.tablename) {
            ...
            Table.find({
                query: {
                    ...
                    query: JSON.stringify({query:JSON.parse(this.query)}),
                },
            }).then(async (res) => {
