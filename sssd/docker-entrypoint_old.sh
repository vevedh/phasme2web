...
GUEST_PASSWORD=${GUEST_PASSWORD:-d@nZel!77}
...
echo --------------------------------------------------
echo 'Registering credential in /etc/krb5.keytab'
echo --------------------------------------------------
...
echo -------------------------------------------------
/usr/bin/pwsh -f /srv/pwrshell/AddDnsRecordA.ps1 -Username "$AD_USERNAME" -Pass "$AD_PASSWORD" -Dc "$ADMIN_SERVER" -Computer "$HOSTNAME" -Ip "$IP_ADDRESS"
