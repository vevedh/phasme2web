    ...
    <div class="">
            ...
            <template v-slot:top="props">
                <div class="col-12 col-md full-width ">
                    <div class="row q-gutter-sm text-h6 q-px-sm ">
                        ...
                        <span>&nbsp;</span>
                        <!--<span class="row items-center text-h8  q-pa-none">
                            accueil PARKING &nbsp;<q-avatar color="teal" size="35px" font-size="20px"  text-color="white" >{{
                                    nbParking
                            }}</q-avatar>
                        </span>-->
                            ...
                            <q-card style="width: 80%; max-width: 100%">
                                ...
                                <q-card-actions align="center" class="q-pa-md">
                                    <div class=" row full-width q-gutter-xl items-center justify-center">
                                        ...
                                        <q-btn label="Modifier" color="primary"  @click="updateRow"v-close-popup>
                                        </q-btn>
                                    </div>
                                </q-card-actions>
                            </q-card>
        ...
        <q-dialog v-model="show_AddDialog">
                            <q-card style="width: 80%; max-width: 100%">
                                ...
                                <q-card-section class="q-pa-md" style="width: 100%; max-width: 100%">
                                    <f-vv-form-by-name ref="addForm" :nameForm="'Accueil'"  :fieldsmodel="{chargeaccueil: currentUsername}"
                                        :hiddenFields="[{ chargeaccueil: currentUsername }]" class="q-pa-sm" @onFormAdded="addRow" :showSave="true" :saveLabel="'Enregistrer'">
                                    </f-vv-form-by-name>
                                </q-card-section>
...
export default defineComponent({
    ...
    computed: {
        ...
        liveDatas() {
            //  si non table d ela base de donnée
            if (!this.dbtype && !this.dbname) {
                return [...this.datas];
            }

            // si il ya un filtre actif
            if (Array.isArray(this.customFilter)) {
                return this.customFilter; //
            } else {
                // donnees sans filtres
                return this.data;
            }
        },
        useSelection() {
        ...
        nbRdc() {
            return this.liveDatas.filter((item) => item.typeacces == 'RDC' && item.etat != 'parking' ).length;
        },
        nbParking() {
            return this.liveDatas.filter((item) => item.typeacces == 'PARKING' && item.etat == 'parking').length; // this.liveDatas.filter(item => item.typeacces == "PARKING" && item.etat == "PRESENT" ).length
        },
        nbResParking() {
            //return this.liveDatas.filter(item => item.etat == "ATTENTE").length
        },
        useSelected: {
            get() {
                if (this.editable || this.selectable) {
        ...
        visiteursDuJour() {
            if (
                !this.$store.state.admin.visiteurs ||
                !Array.isArray(this.$store.state.admin.visiteurs)
            )
                return;
            return this.$store.state.admin.visiteurs
                .filter(
                    (elt) => (elt.datevisite == this.datedujour) && (elt.etat != 'attente') && (elt.etat != 'efface') && (elt.typeacces=='RDC')
                )
                .sort((a, b) => {
    ...
    methods: {
        ...
        /**
         *   MIse à jour des données pour filtrer entetes de colonnes
         */
        ...
        async addRow(addedField) {
            if (this.newColumns.length == 0) {
                return;
            }
            
            this.editRow = addedField;
            console.log('Add rows :', this.editRow);
        ...
        refreshDatas() {
            ...
            if (this.dbtype && this.dbname) {
                Table.find({
                    ...
                    this.data = Object.assign([],res.map((o) =>(!Object(o).hasOwnProperty('etat'))?({...o,etat:''}):({...o})).filter(
                        (o) => ((o.etat != 'attente') && (o.etat != 'efface') && (o.etat != 'sortie')) 
                    ))
                    .map((obj) => {return Object.assign({},{...obj},{contactinterne:String(obj.contactinterne).split('[')[0].trim()})})
                    //.map((obj) => {return Object.assign({},{...obj},{datevisite:(String(obj.datevisite.split('/')[2]).length==2)?'20'+obj.datevisite.split('/')[2]:obj.datevisite.split('/')[2]+obj.datevisite.split('/')[1]+obj.datevisite.split('/')[0],contactinterne:String(obj.contactinterne).split('[')[0].trim()})})
                    .sort((a, b) => {
                        var dateA = date.formatDate(a.datevisite,'YYYYMMDD'); //new Date(a.date_prop).getTime();
                        var dateB = date.formatDate(b.datevisite,'YYYYMMDD'); //new Date(b.date_prop).getTime();
                        return dateA < dateB ? 1 : -1; // ? -1 : 1 for ascending/increasing order
                    });
                    await this.updateDatas();
                });
        ...
        async updateDatas() {
            let tmpFormDatas;
            let formFields;
            /*const visiteurs = await feathersClient.service('tables').find({
                query: {
                    tableDb: this.dbtype,
                    tableName: this.dbname,
            ...
            const tmpdatas = visiteurs.filter(
                (o) => (o.etat != 'attente') || (o.etat != 'efface') || (o.etat != 'sortie') || !Object(o).hasOwnProperty('etat')
            )
            console.log('Table result info:', tmpdatas);*/
