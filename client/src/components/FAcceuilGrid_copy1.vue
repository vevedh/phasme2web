<template>
    <div class="">
        ...
        <q-table id="myTable" ref="myAccTable" :title="setTitle" :data="liveDatas"
            ...
            :selected.sync="useSelected" :visible-columns="visibleColumns" :pagination="initialPagination"
            :separator="'cell'">
            ...
            <template v-slot:top="props">
                <div class="col-12 col-md full-width ">
                    <div class="row q-gutter-sm text-h6 q-px-sm ">
                        ...
                        <q-space />
                        <span class="row items-center text-h8  q-pa-none"><q-avatar color="blue" size="35px" font-size="20px"  text-color="white">{{ nbRdc }}</q-avatar> &nbsp;Visiteurs accueil RDC
                           
                        </span>
                        <span>&nbsp;</span>
                        <span class="row items-center text-h8  q-pa-none">
                            <q-avatar color="teal" size="35px" font-size="20px"  text-color="white" >{{nbParking}}</q-avatar>&nbsp;accueil PARKING 
                        </span>
                        <span class="row items-center text-h8  q-pa-none" v-if="nbResParking!=null"><q-avatar color="amber" size="35px" font-size="20px"  text-color="black" >{{nbResParking}}</q-avatar> &nbsp;demandes PARKING &nbsp;</span>
                    </div>
                    ...
                    <div class="row q-gutter-sm q-px-sm q-pb-md justify-center">
                        <q-btn-dropdown color="primary">
                            <template v-slot:label>
                                <div class="row items-center no-wrap">
                                    <q-icon left name="filter_alt" :color="
                                        currentAcces == '' || currentAcces == null
                                            ? 'white'
                                            : 'red'
                                    " />
                        ...
                        <q-btn dense no-caps filled shadow-4
                            ...
                            <div class="row items-center no-wrap">
                                <q-icon left name="filter_alt" :color="
                                    dateSelected && dateSelected != '' ? 'red' : 'primary'
                                " />
                                <div class="text-center">
                                    Visites pour
                                    <span :class="dateSelected && dateSelected != ''? 'text-bold text-h6 text-red': null">{{dateSelected && dateSelected != ''? dateSelected: 'la date'}}</span>
                                    (cliquez-ici)
                                </div>
                            </div>
                        ...
                        <q-btn icon="filter_alt_off" color="primary" no-caps label="Effacer les filtres" @click="deleteFilters()">
                            <q-tooltip content-class="text-dark text-subtitle2 text-center bg-amber">
                    ...
                    <div v-if="allowexport" class="row q-gutter-sm q-pb-md">
                        ...
                        <q-btn dense no-caps flat filled class="bg-primary" text-color="white" icon="far fa-file-pdf"
                            label=".pdf" @click="exportDatasPdf()" />
                    </div>
        ...
        <q-dialog v-model="show_EditDialog">
                            <q-card style="width: 80%; max-width: 100%">
                                ...
                                <q-card-section class="q-pa-md" style="width: 100%; max-width: 100%">
                                    <f-vv-form-by-name ref="editForm" :nameForm="(selectedRow.typeacces=='PARKING')?'Demande Parking':'Accueil'"  :fieldsmodel="selectedRow"
                                        :hiddenFields="[{ chargeaccueil: currentUsername }]" class="q-pa-sm">
                                    </f-vv-form-by-name>
                                </q-card-section>
                                <q-card-actions align="center" class="q-pa-md">
                                    <div class=" row full-width q-gutter-xl items-center justify-center">
                                        <q-btn label="Annuler" color="primary" v-close-popup
                                            @click="show_EditDialog = false"></q-btn>
...
import FVvFormByName from './FVvFormByName.vue';

export default defineComponent({
    components: { FVvForm, DiagTimeLineJour, FVvFormByName },
    name: 'FAcceuilGrid',
    data() {
        const myLocale = {
    ...
    computed: {
        ...
        /*nbRdcTotal() {
            return this.totalVisiteurs.filter((item) => item.typeacces == 'RDC' && ((item.etat != 'efface') || (item.etat != 'sortie') || (item.etat != 'parking')  || (item.etat != 'attente'))).length;
        },
        nbParkingTotal() {
            return this.totalVisiteurs.filter((item) => item.typeacces == 'PARKING' && (item.etat == 'parking')).length; // this.liveDatas.filter(item => item.typeacces == "PARKING" && item.etat == "PRESENT" ).length
        },*/
        nbRdc() {
            if (!this.liveDatas)
                return;
            return this.liveDatas.filter((item) => item.typeacces == 'RDC' && ((item.etat != 'efface') || (item.etat != 'sortie'))  ).length;
        },
        nbParking() {
            if (!this.liveDatas)
                return;
            return this.liveDatas.filter(item => item.typeacces == "PARKING" && item.etat == "parking" ).length
        },
        nbResParking() {
            if (!this.liveDatas)
                return;
            return this.liveDatas.filter((item) => ( item.typeacces == 'PARKING') && item.etat == 'attente').length;
        },
        useSelected: {
            get() {
                if (this.editable || this.selectable) {
...
return this.liveDatas.filter(
    ...
    methods: {
        ...
        compDate(datej){
           let cdate = new Date(
            ...
            return Number(String(datej.split('/')[2]+datej.split('/')[1]+datej.split('/')[0]))
        },
         ...
         *  delete filter
         */
        async deleteFilters(objScope, headerToFiltre) {
            console.log('delete filtres')
            this.currentAcces = '';
            this.dateSelected = null;
            this.currentDateSel = null;
            this.dateRange = null;
            this.filters[`${headerToFiltre}`] = '';
            this.filters=Object.assign({},{});
            this.customFilter = this.filterArray(this.datas, this.filters);
        ...
        async addFormDatas(fieldsValues) {
            this.$q.loading.show({
                spinner: QSpinnerGears,
                spinnerColor: 'red',
                message: 'Chargement en cours...',
            });
            await this.addVisiteur(fieldsValues);
            this.$q.loading.hide();
        ...
        async updateDatas() {
            ...
            if (this.datas.length>0) {
                tmpdatas = Object.assign([],this.datas);
                //this.data = Object.assign([],this.datas);
            } /*else {
                tmpdatas = await feathersClient.service('tables').find({
                query: {
                    tableDb: this.dbtype,
                    tableName: this.dbname,
            ...
            /*console.log('Table result info:', tmpdatas.map((o) =>(!Object(o).hasOwnProperty('etat'))?({...o,etat:''}):({...o})));
            this.totalVisiteurs = Object.assign([],tmpdatas.map((o) =>(!Object(o).hasOwnProperty('etat'))?({...o,etat:''}):({...o})).filter(
                (o) => ((o.etat != 'attente') || (o.etat != 'efface') || (o.etat != 'sortie')) 
            ));
            this.data = Object.assign([], tmpdatas.map((o) =>(!Object(o).hasOwnProperty('etat'))?({...o,etat:''}):({...o})).filter(
                (o) => ((o.etat != 'attente') || (o.etat != 'efface') || (o.etat != 'sortie')) 
            ));*/
            console.log('Table result info data :', this.data);
            if (this.data.length > 0) {
                const countkeys = this.data.map((o) => `${Object.keys(o).length}`);
                ...
                const dbdata = Object.assign([], this.data);
            ...
            //this.filters['dateentree'] = this.whereDateYearEq('2021')//(item) => item.split('/')[2] === '2020'
            //this.filters['usercacem'] = this.whereContain('Herve')
            this.customFilter =this.filterArray(this.data, this.filters).sort((a, b) => {
                    var dateA = this.compDate(a.datevisite); //new Date(a.date_prop).getTime();
                    var dateB = this.compDate(b.datevisite); //new Date(b.date_prop).getTime();
                    return dateA < dateB ? 1 : -1; // ? -1 : 1 for ascending/increasing order
                });
    ...
    beforeMount() {
        ...
        if (this.dbtype && this.dbname) {
            ...
            console.log('Données', this.datas);
            this.data =this.datas;
            await this.updateDatas();
        }*/
        console.log('Données', this.datas.filter(
                (o) => ((o.etat != 'attente') || (o.etat != 'efface') || (o.etat != 'sortie')) 
            ));
    },
    async mounted() {
        console.log('Données', this.datas.filter(
            ...
            this.data = Object.assign([],this.datas.filter(
                (o) => ((o.etat != 'attente') || (o.etat != 'efface') || (o.etat != 'sortie')) 
            ));
            await this.updateDatas();
