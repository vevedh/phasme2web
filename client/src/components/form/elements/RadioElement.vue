<template>
  <div class="q-px-sm q-form-radio q-form-builder-element">
    <div class="row">
    ...
    <div class="row">
      <q-list dense>
        ...
        <q-item dense v-if="includeOther">
          <q-item-section avatar><q-radio v-model="innerValue" label="Autre :" :val="otherOptionVal"  keep-color :color="hasError ? 'red' : ''" :ref="`radio-other-${id}`" @input="validate" /></q-item-section>
  ...
  methods: {
    ...
    validate () {
      ...
      if (this.required && !this.innerValue) {
        this.hasError = true
        this.errors.push('Svp sélectionner une option.')
      }
    }
  }
}
