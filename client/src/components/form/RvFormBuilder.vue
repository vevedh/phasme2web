  ...
  <q-layout view="hHh LpR fFf">
    ...
    >
      ...
      >
        ...
        <div
          class="source-field q-gutter-sm row no-wrap"
      ...
      <div class="q-pa-none q-gutter-md">
        ...
        <q-separator class="q-my-md" />
        <q-expansion-item dense expand-separator icon="perm_identity" label="Champs cachés" class="text-center tex-bold bg-blue-3">
        <q-card>

        <q-card-section class="q-py-none bg-blue-1" v-for="(hField, idx) in hiddenFields"
          :key="`${idx}_hidden`">
...
export default {
  ...
  methods: {
    ...
    saveForm () {
      ...
      if (
        this.nameForm != '' &&
        this.nameForm != 'accueil' &&
        this.nameForm != 'parking'
      ) {
        nameForm = 'autre';
      }
      this.$q
              ...
              .dialog({
                ...
                prompt: {
                  ...
                  isValid: (val) =>
                    val.length > 2 &&
                    val.length < 16 &&
                    /^[A-Za-z0-9]+$/.test(val) == true &&
                    /accueil/.test(val.toLowerCase()) == false &&
                    /parking/.test(val.toLowerCase()) == false, // << here is the magic
                  type: 'text',
                },
                cancel: true,
                persistent: true,
    ...
    onList () {
      let val = Object.assign([], this.vFields);
    ...
    addHiddenField() {
      const fieldHidden = {
        ...
        this.hiddenFields.push(fieldHidden)
        this.currentHiddenField = fieldHidden
    },
  ...
  beforeMount () {
    ...
    this.hiddenFields = this.fields.filter(obj => obj.field_type=='hidden')
    this.vFields = this.fields.filter(obj => obj.field_type!='hidden')
    console.log('Fields :', JSON.stringify(this.vFields))
    /*else {
  ...
  watch: {
    fields (val) {
      ...
      this.hiddenFields = val.filter(obj => obj.field_type=='hidden')
      //this.vFields = val.filter(obj => obj.type!='hidden')
      this.$emit('input',val);
    },
    /*hiddenFields (val) {


      console.log('Form Hidden field change :', val);
      this.$emit('hiddenChange', val);
    },*/
    value (val) {
      this.fields = val;
      this.hiddenFields = this.fields.filter(obj => obj.field_type=='hidden')
